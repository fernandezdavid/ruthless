...
Ruthless doesn't just run when invoked. Once installed on a project, it becomes the PM layer — shaping how GPT behaves on every product-adjacent decision, even when `$ruthless` isn't being explicitly called. This file defines how that governance is installed, verified, and maintained.
