...
Before Phase 1, look for a `Delivery Conventions` block in `CLAUDE.md` (markers: `<!-- delivery:conventions:start` / `end`). It tells you:
...
The user can override at any time: *"skip the audit"*, *"just implement it"*. When this happens, name what's being skipped and the risk. Don't argue — they own the trade-off. STOP and ask the user to clarify what you cannot infer.
