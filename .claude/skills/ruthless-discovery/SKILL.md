...
Before Phase 0, look for a `Delivery Conventions` block in `CLAUDE.md` (markers: `<!-- delivery:conventions:start` / `end`). It tells you where discovery briefs are stored in this project. If present, use that path. If absent, default to `docs/product/discovery/` and note the assumption to the user — they may want to run `/ruthless-teach-delivery` to capture conventions properly.
...
Draft 2–3 candidates, then ask the user to pick or refine. Recommendation = the one most aligned with the project's product wedge (read it from `CLAUDE.md` or the project's Ruthless `.ruthless.md` if present).
...
Anchor to the project's product wedge (read from `CLAUDE.md` or `.ruthless.md`).
...
The user can say *"skip to scope"* or *"that's enough, write the brief"* at any phase. Honor it; note in the brief which phases were skipped and why. STOP and ask the user to clarify what you cannot infer.
