...
- **First time** running `{{command_prefix}}ruthless-ship`, `{{command_prefix}}ruthless-discovery`, or `{{command_prefix}}ruthless-design` in a new project.
- After a major workflow change (new design system, new test framework, switching issue trackers).
- Re-run with `{{command_prefix}}ruthless-teach-delivery force` to refresh after substantive changes.
...
1. **`{{config_file}}`** — check for existing `Delivery Conventions` block (markers below). Also check for an Impeccable design-context section to reference instead of re-asking.
...
Write a clearly-marked block to `{{config_file}}` (create the file if absent). Use these markers — the `ruthless-ship` family looks for them:

```markdown
<!-- delivery:conventions:start v1 -->
## Delivery Conventions

> Captured by `{{command_prefix}}ruthless-teach-delivery` on YYYY-MM-DD. Re-run with `{{command_prefix}}ruthless-teach-delivery force` after substantive workflow changes.
...
- Design context source: <`{{command_prefix}}teach-impeccable` output in this file | dedicated brand doc at `<path>` | none captured>
...
- **`{{command_prefix}}ruthless teach`** runs first (strategic context → `.ruthless.md`). At the end of its run, it should offer to chain into `{{command_prefix}}ruthless-teach-delivery` if no Delivery Conventions block exists.
- **`{{command_prefix}}teach-impeccable`** (if installed) handles design context. Read its output if present; recommend running it if absent but don't require it.

The three teach skills are independent but complementary:

```
ruthless teach              → why we ship          → .ruthless.md
teach-impeccable            → how it looks         → {{config_file}}#impeccable
ruthless-teach-delivery     → how we ship          → {{config_file}}#delivery
```
...
End by writing the block to `{{config_file}}` and confirming with the user. Mention next steps:

> *"Delivery Conventions written to `{{config_file}}`. The `{{command_prefix}}ruthless-ship` / `{{command_prefix}}ruthless-discovery` / `{{command_prefix}}ruthless-design` skills will now read this block at startup. Run `{{command_prefix}}ruthless-teach-delivery force` to refresh after major workflow changes."*

## How to exit early

The user can say *"skip section X"* or *"that's enough, write what we have"*. Honor it; mark skipped sections as `TODO` in the output block so they're easy to fill later. {{ask_instruction}}
