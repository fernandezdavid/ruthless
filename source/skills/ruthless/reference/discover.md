...
> **This is project-level discovery, not feature-level.** For per-feature discovery (job-to-be-done, audit, signal, scope for one specific feature inside an already-positioned product), see `{{command_prefix}}ruthless-discovery` in the ship family.
