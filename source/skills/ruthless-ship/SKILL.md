...
- **Block absent**: offer to run `{{command_prefix}}ruthless-teach-delivery` first:
  ...
  > *"No Delivery Conventions block in `{{config_file}}` yet. Running `{{command_prefix}}ruthless-teach-delivery` first (10 min) will save context across this and future ship flows. Run it now, or proceed with sensible defaults and you'll fill in as we go?"*
...
USER REQUEST
    ↓
{{command_prefix}}ruthless-discovery          [Phases 0–9]
    ↓
<feature>-discovery.md (in project's discovery directory)
    ↓
[USER SIGN-OFF GATE]
    ↓
{{command_prefix}}ruthless-design             [Phases 1–7]
    ↓
PR + screenshots + ADR
```
...
2. **Invoke discovery**: run `{{command_prefix}}ruthless-discovery` end-to-end. The output is a committed brief.
...
4. **Invoke design flow**: run `{{command_prefix}}ruthless-design` with the discovery brief as the Phase 1 (FRAME) input. The skill reads the brief, doesn't re-frame.
...
- **Resuming**: if a discovery brief already exists, jump straight to `{{command_prefix}}ruthless-design` and pass the existing brief.
