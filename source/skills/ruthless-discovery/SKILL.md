...
**Output**: a committed brief at the project's discovery directory (typically `docs/product/discovery/<feature>-discovery.md`) — feeds directly into `{{command_prefix}}ruthless-design` Phase 1 (FRAME).

**Companion**: `{{command_prefix}}ruthless-design` for design + build phases.
...
Draft 2–3 candidates, then ask the user to pick or refine. Recommendation = the one most aligned with the project's product wedge (read it from `{{config_file}}` or the project's Ruthless `.ruthless.md` if present).
...
- `{{command_prefix}}audit` for comprehensive evaluation (accessibility, performance, theming, responsive)
- `{{command_prefix}}critique` for a UX read on the existing surface
...
Anchor to the project's product wedge (read from `{{config_file}}` or `.ruthless.md`).

Use `{{command_prefix}}positioning-messaging` for the heavy lifting.
...
2. **Reddit** via `{{command_prefix}}reddit-scout` — what users say about this problem space
3. **User interviews** via `{{command_prefix}}conducting-user-interviews` — only if the gap is big enough to warrant a live conversation
...
**Template** (matches `{{command_prefix}}ruthless-design` Phase 1 input):

```markdown
# <Feature> — Discovery

> Status: discovery complete · ready for {{command_prefix}}ruthless-design
...
**Gate**: file exists, all sections populated (or marked N/A with rationale), user has signed off. Then either run `{{command_prefix}}ruthless-design` next, or hand back control.
...
The user can say *"skip to scope"* or *"that's enough, write the brief"* at any phase. Honor it; note in the brief which phases were skipped and why. {{ask_instruction}}
