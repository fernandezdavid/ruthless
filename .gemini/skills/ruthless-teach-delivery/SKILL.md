...
A one-time interview that captures the *how-we-ship* layer of a project. Strategic context (why, for whom) lives in `.ruthless.md` via `/ruthless teach`. Design context (palette, typography, voice) lives in `GEMINI.md` via `/teach-impeccable` if installed. **This skill fills the gap between them**: the project conventions that delivery work depends on — where things live, how they're tested, how they ship.
...
1. **`GEMINI.md`** — check for existing `Delivery Conventions` block (markers below). Also check for an Impeccable design-context section to reference instead of re-asking.
...
Write a clearly-marked block to `GEMINI.md` (create the file if absent). Use these markers — the `ruthless-ship` family looks for them:
...
teach-impeccable            → how it looks         → GEMINI.md#impeccable
ruthless-teach-delivery     → how we ship          → GEMINI.md#delivery
```
...
End by writing the block to `GEMINI.md` and confirming with the user. Mention next steps:

> *"Delivery Conventions written to `GEMINI.md`. The `/ruthless-ship` / `/ruthless-discovery` / `/ruthless-design` skills will now read this block at startup. Run `/ruthless-teach-delivery force` to refresh after major workflow changes."*
