...
Once installed, Ruthless doesn't just run when invoked — it becomes the project's PM layer. It installs a governance block into `GEMINI.md` so that every product-adjacent decision (new features, scope changes, positioning, roadmap, prioritization) gets filtered through Ruthless principles and anti-patterns *even when `/ruthless` isn't explicitly called*. Subagents inherit the governance. The project stays on the rails.
...
Gemini will happily generate 47 features, a 12-page PRD, and an implementation plan — without ever asking "should we build this at all?" The result is Frankenstein products: internally coherent code, externally incoherent value.
...
| `/ruthless teach` | Capture product, customer, market, and constraint context (writes to `.ruthless.md`). **Also installs the Ruthless governance block in `GEMINI.md` by default** so the project stays on the rails between explicit invocations. |
...
2. **GEMINI.md** — if present, may contain a Product Context section and/or the Ruthless governance block (see Step 1.2).
...
Check `GEMINI.md` for the Ruthless governance markers:
...
- **Both markers present, version current:** governance is installed and up to date. Mention it briefly ("Ruthless governance block found in `GEMINI.md`, v2 — active.") and continue.
...
- **Markers missing and `.ruthless.md` has real content:** Ruthless was installed before but governance was removed (or never installed). Offer to (re)install: "No Ruthless governance block in `GEMINI.md`. Install it so the project stays on the rails between `/ruthless` invocations?"
...
When teach writes `.ruthless.md`, it **also installs the governance block** in `GEMINI.md` by default (see [reference/governance.md](reference/governance.md)). Tell the user you're doing this and why:

> I'm installing the Ruthless governance block in `GEMINI.md`. This makes Ruthless the persistent PM layer for this project — so every product-adjacent decision gets filtered through the principles and anti-patterns, even when `/ruthless` isn't explicitly invoked. You can view or edit it between the `ruthless:governance:start` / `end` markers, and remove it by deleting the block.
...
**Critical quality check at every stage**: If you showed what you're producing — PRD, pitch, landing page, MVP scope — to ten other operators and said "Gemini helped with this," would they believe you immediately? If yes, that's the problem.
...
**Then install the governance block** in `GEMINI.md` (see [reference/governance.md](reference/governance.md) for the canonical template and install logic). This is not optional — it's how Ruthless becomes the persistent PM layer for the project. Follow the install procedure exactly:

1. Check for existing `<!-- ruthless:governance:start` / `end` markers.
2. If absent, append the governance block (with the current `v1` marker) to the end of `GEMINI.md` (create the file if it doesn't exist).
...
Ruthless captures *why* we ship. There's a sibling skill that captures *how* we ship — paths, primitives, test conventions, workflow tooling. The `ruthless-ship` family of skills (`ruthless-ship`, `ruthless-discovery`, `ruthless-design`) reads from a `Delivery Conventions` block in `GEMINI.md` that this sibling installs.

Check `GEMINI.md` for `<!-- delivery:conventions:start` markers:
